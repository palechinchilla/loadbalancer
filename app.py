...
# ---------------------------------------------------------------------------
# Configuration (preserved from handler.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ComfyUI helper functions (ported from handler.py)
# ---------------------------------------------------------------------------
...
def validate_input(job_input):
    """
    Validates the input for the generate endpoint.
...
async def process_workflow(request_data, request_id):
    ...
    This is the async equivalent of the old handler() function.
    """
    # Validate input (Pydantic already validated structure, but this
    # catches workflow-specific issues and normalises the data)
    input_dict = request_data.model_dump()
    validated_data, error_message = validate_input(input_dict)
    if error_message:
    ...
    if input_images:
        upload_result = await asyncio.to_thread(upload_images, input_images)
                ...
                if image_bytes:
                    ...
                    if os.environ.get("BUCKET_ENDPOINT_URL"):
                        try:
                            ...
                            print(
                                f"worker-comfyui - [{request_id}] Wrote image bytes to temporary file: {temp_file_path}"
                            )

                            print(f"worker-comfyui - [{request_id}] Uploading {filename} to S3...")
                            s3_url = await asyncio.to_thread(
                                upload_to_s3, request_id, temp_file_path
                            )
                            os.remove(temp_file_path)
                            print(
                                f"worker-comfyui - [{request_id}] Uploaded {filename} to S3: {s3_url}"
                            )
                            output_data.append(
                                {
                                    "filename": filename,
                            ...
                            if "temp_file_path" in locals() and os.path.exists(
                                ...
                                try:
                                    ...
                                    print(
                                        f"worker-comfyui - [{request_id}] Error removing temp file {temp_file_path}: {rm_err}"
                                    )
                    else:
                        try:
...
async def _wait_for_comfyui(app_instance):
    ...
    # Run network volume diagnostics at startup if enabled
